...
int main(int argc, char* argv[])
{
  ...
  if(iTaskID == MASTER)
  {
    ...
    for(i=1; i<=iNumWorker; i++)
    {
      ...
      iFrom = (i * sMgrid.iRow) / iNumWorker;
      iTo = ((i + 1) * sMgrid.iRow) / iNumWorker;
      iRows = iFrom - iTo;
      iCols = iSize;
  ...
  else
  {
    ...
    /* Recieve data */
    MPI_Recv(&(sMgridTmp.ppaMat[1][0]), iRows*iCols, MPI_DOUBLE, MASTER, BEGIN, MPI_COMM_WORLD, &mpiStatus);
	...
	{
          sMgrid.ppaMat[j][k]=sMgridTmp.ppaMat[j+1][k];
          sMgrid.ppaMat[j][k]+=FI*((-4)*sMgridTmp.ppaMat[j+1][k]+sMgridTmp.ppaMat[j+2][k]+sMgridTmp.ppaMat[j][k]+sMgridTmp.ppaMat[j+1][k+1]+sMgridTmp.ppaMat[j][k-1]);
        }
      if(iLeftNeighbor != 0)
      {
        j=0;
