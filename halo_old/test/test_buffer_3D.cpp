...
//   std::vector<int> iplusjplus_r( t+i+j+2 );
  ...
  /* Initializing receive buffers */
    /*Direct neighbours */
  tb.init_recv_buffer(&kplus_r[0], MPI_INT, ( t+i+j+k ), 0,0,1);
  tb.init_recv_buffer(&kminus_r[0], MPI_INT, ( t+i+j+k ), 0,0,-1);
  tb.init_recv_buffer(&iplus_r[0], MPI_INT, ( t+i+j+k ), 1,0,0);
  tb.init_recv_buffer(&iminus_r[0], MPI_INT, ( t+i+j+k ), -1,0,0);
  tb.init_recv_buffer(&jplus_r[0], MPI_INT, ( t+i+j+k ), 0,1,0);
  tb.init_recv_buffer(&jminus_r[0], MPI_INT, ( t+i+j+k ), 0,-1,0);
  
  /*Rest of the k=-1 plane */
  tb.init_recv_buffer(&iplusjpluskminus_r[0], MPI_INT, ( t+i+j+k ), 1,1,-1);
  tb.init_recv_buffer(&jpluskminus_r[0], MPI_INT, ( t+i+j+k ), 0,1,-1);
  tb.init_recv_buffer(&iminusjpluskminus_r[0], MPI_INT, ( t+i+j+k ), -1,1,-1);
  tb.init_recv_buffer(&ipluskminus_r[0], MPI_INT, ( t+i+j+k ), 1,0,-1);
  tb.init_recv_buffer(&iminuskminus_r[0], MPI_INT, ( t+i+j+k ), -1,0,-1);
  tb.init_recv_buffer(&iplusjminuskminus_r[0], MPI_INT, ( t+i+j+k ), 1,-1,-1);
  tb.init_recv_buffer(&jminuskminus_r[0], MPI_INT, ( t+i+j+k ), 0,-1,-1);
  tb.init_recv_buffer(&iminusjminuskminus_r[0], MPI_INT, ( t+i+j+k ), -1,-1,-1);
  
  /*Rest of the k=0 plane */
  tb.init_recv_buffer(&iplusjplus_r[0], MPI_INT, ( t+i+j+k ), 1,1,0);
  tb.init_recv_buffer(&iminusjplus_r[0], MPI_INT, ( t+i+j+k ), -1,1,0);
  tb.init_recv_buffer(&iplusjminus_r[0], MPI_INT, ( t+i+j+k ), 1,-1,0);
  tb.init_recv_buffer( &iminusjminus_r[0], MPI_INT, ( t+i+j+k ), -1,-1,0);
  
  /*Rest of the k=1 plane */
  tb.init_recv_buffer(&iplusjpluskplus_r[0], MPI_INT, ( t+i+j+k ), 1,1,1);
  tb.init_recv_buffer(&jpluskplus_r[0], MPI_INT, ( t+i+j+k ), 0,1,1);
  tb.init_recv_buffer(&iminusjpluskplus_r[0], MPI_INT, ( t+i+j+k ), -1,1,1);
  tb.init_recv_buffer(&ipluskplus_r[0], MPI_INT, ( t+i+j+k ), 1,0,1);
  tb.init_recv_buffer(&iminuskplus_r[0], MPI_INT, ( t+i+j+k ), -1,0,1);
  tb.init_recv_buffer(&iplusjminuskplus_r[0], MPI_INT, ( t+i+j+k ), 1,-1,1);
  tb.init_recv_buffer(&jminuskplus_r[0], MPI_INT, ( t+i+j+k ), 0,-1,1);
  tb.init_recv_buffer(&iminusjminuskplus_r[0], MPI_INT, ( t+i+j+k ), -1,-1,1);
