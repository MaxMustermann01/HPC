...
int main(int argc, char** argv) {
  ...
  Proc_Grid_t<2> proc_grid(CartComm);
  
  TransmitBuffer<2, 8> tb(proc_grid);
  
  proc_grid.coords(i, j);
  proc_grid.dims(N, M);
  ...
  tb.init_send_buffer<-1,-1>(&iminusjminus[0], MPI_INT, (k+i+j));
  tb.init_send_buffer<-1, 1>(&iminusjplus[0], MPI_INT, (k+i+j));
  tb.init_send_buffer< 1,-1>(&iplusjminus[0], MPI_INT, (k+i+j));
  tb.init_send_buffer< 1, 1>(&iplusjplus[0], MPI_INT, (k+i+j));
  tb.init_send_buffer<-1, 0>(&iminus[0], MPI_INT, (k+i+j));
  tb.init_send_buffer< 1, 0>(&iplus[0], MPI_INT, (k+i+j));
  tb.init_send_buffer< 0,-1>(&jminus[0], MPI_INT, (k+i+j));
  tb.init_send_buffer< 0, 1>(&jplus[0], MPI_INT, (k+i+j));

  tb.init_recv_buffer<-1,-1>(&iminusjminus_r[0], MPI_INT, (k+i+j-2));
  tb.init_recv_buffer<-1, 1>(&iminusjplus_r[0], MPI_INT, (k+i+j));
  tb.init_recv_buffer< 1,-1>(&iplusjminus_r[0], MPI_INT, (k+i+j));
  tb.init_recv_buffer< 1, 1>(&iplusjplus_r[0], MPI_INT, (k+i+j+2));
  tb.init_recv_buffer<-1, 0>(&iminus_r[0], MPI_INT, (k+i+j-1));
  tb.init_recv_buffer< 1, 0>(&iplus_r[0], MPI_INT, (k+i+j+1));
  tb.init_recv_buffer< 0,-1>(&jminus_r[0], MPI_INT, (k+i+j-1));
  tb.init_recv_buffer< 0, 1>(&jplus_r[0], MPI_INT, (k+i+j+1));
  ...
  std::fill(&res_iminus_r[0], &res_iminus_r[k+i+j-1], proc_grid.proc<-1,0>());
  std::fill(&res_iplus_r[0], &res_iplus_r[k+i+j+1], proc_grid.proc<1,0>());
  std::fill(&res_jminus_r[0], &res_jminus_r[k+i+j-1], proc_grid.proc<0,-1>());
  std::fill(&res_jplus_r[0], &res_jplus_r[k+i+j+1], proc_grid.proc<0,1>());
  std::fill(&res_iminusjminus_r[0], &res_iminusjminus_r[k+i+j-2], proc_grid.proc<-1,-1>());
  std::fill(&res_iplusjplus_r[0], &res_iplusjplus_r[k+i+j+2], proc_grid.proc<1,1>());
  std::fill(&res_iplusjminus_r[0], &res_iplusjminus_r[k+i+j], proc_grid.proc<1,-1>());
  std::fill(&res_iminusjplus_r[0], &res_iminusjplus_r[k+i+j], proc_grid.proc<-1,1>());
